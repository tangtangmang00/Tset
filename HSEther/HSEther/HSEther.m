...
+(void)hs_inportMnemonics:(NSString *)mnemonics pwd:(NSString *)pwd block:(void(^)(NSString *address,NSString *keyStore,NSString *mnemonicPhrase,NSString *privateKey,BOOL suc,HSWalletError error,NSString *message))block{
    if (mnemonics.length < 1) {
        block(@"",@"",@"",@"",NO,HSWalletErrorMnemonicsLength,@"助记词长度不够");
        return;
    }
    if (pwd.length < 1) {
        block(@"",@"",@"",@"",NO,HSWalletErrorPwdLength,@"助记词个数不够");
        return;
    }
    ...
    for (NSString *m in arrayMnemonics) {
        if (![Account isValidMnemonicWord:m]) {
            ...
            NSLog(@"%@",msg);
            block(@"",@"",@"",@"",NO,HSWalletErrorMnemonicsValidWord,msg);
            return;
        }
    }
...
+(void)hs_importWalletForPrivateKey:(NSString *)privateKey
                                pwd:(NSString *)pwd
                              block:(void(^)(NSString *address,NSString *keyStore,NSString *mnemonicPhrase,NSString *privateKey,BOOL suc,HSWalletError error,NSString *message))block{
    if (privateKey.length < 1) {
        block(@"",@"",@"",@"",NO,HSWalletErrorPrivateKeyLength,@"私钥长度不够");
        return;
    }
    if (pwd.length < 1) {
        block(@"",@"",@"",@"",NO,HSWalletErrorPwdLength,@"密码长度不够");
        return;
    }
    //1 解密私钥
...
//    address = @"0xe83aec696478bba2404eaa0eb9b1f2d58269d637";
...
//    EtherscanProvider *e = [[EtherscanProvider alloc] initWithChainId:ChainIdHomestead apiKey:@"RCWEX6WYBXMJZHD5FD617NZ99TZADKBEDJ"];
    JsonRpcProvider *e = [[JsonRpcProvider alloc] initWithChainId:ChainIdHomestead url:[NSURL URLWithString:@"https://chain.xmicoin.net/blockchain"]];
    ...
    if (array.count > 1) {
        ...
        [[e getBalance:a] onCompletion:^(BigNumberPromise *promise ) {
            if (!promise.result || promise.error) {
                block([NSArray array],NO);
            } else {
                NSString *string = promise.value.decimalString;
                NSDecimalNumber *value = [NSDecimalNumber decimalNumberWithString:string];
                NSDecimalNumber *plus = [NSDecimalNumber decimalNumberWithString:@"1000000000000000000"];
                NSDecimalNumber *balance = [value decimalNumberByDividingBy:plus];
                NSString *balString = [NSString stringWithFormat:@"%@",balance];
                block([NSArray arrayWithObject:balString],YES);
            }
        }];
    }
...
//    __block EtherscanProvider *e = [[EtherscanProvider alloc]initWithChainId:ChainIdHomestead apiKey:@"RCWEX6WYBXMJZHD5FD617NZ99TZADKBEDJ"];
    ...
    JsonRpcProvider *e = [[JsonRpcProvider alloc]initWithChainId:ChianIdAny url:[NSURL URLWithString:@"https://chain.xmicoin.net/blockchain"]];
    ...
    Transaction *transaction = [Transaction transactionWithFromAddress:[Address addressWithString:addressStr]];
//
    //1 account自己解密
